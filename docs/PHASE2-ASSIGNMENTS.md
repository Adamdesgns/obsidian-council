...
- Tests: `node --test` from the repo root (Node auto-discovers `test/**/*.test.mjs`; a bare directory argument does not work on this Node). Fake members (`test/fake-member.mjs`, a script that reads the packet on stdin and prints scripted JSON) drive every test except the two live ones, which are skipped unless `COUNCIL_LIVE=1`.
...
- `src/adapters/{claude,codex,grok}.mjs`: each exports `describe()`, `argsFor(packet, {resume, persist, newId})`, `sessionOf(result)`, `finalText(result)`, `usageOf(result)`. Use exactly the flags proven in the spike: Claude `--print --output-format json --max-turns N --permission-mode default --disallowedTools ... -- <prompt>` with `--resume <session_id>`; Codex review role: `exec --json --ignore-user-config --skip-git-repo-check -s read-only -C <cwd> --color never [--ephemeral] <prompt>` and `exec ... resume <thread_id> <prompt>`; Codex Build role (certified by A3b on 2026-09-14): the same with `-s workspace-write -c windows.sandbox="elevated" -c approval_policy="never" -c sandbox_workspace_write.network_access=false -c sandbox_workspace_write.exclude_tmpdir_env_var=true -c sandbox_workspace_write.exclude_slash_tmp=true`, HOME and TEMP for the run pointed inside the workspace so `node --test` does not touch Adam's AppData, and the adapter must read the effective `sandbox_policy` back from the run's rollout file and refuse the result if it is not `workspace-write`; Grok `-p <prompt> --output-format json --max-turns N --tools read_file --no-memory --no-subagents --disable-web-search --cwd <cwd>` with `-s <uuid>` to create and `--resume <uuid>` to continue. **Never `-s` to continue.**
